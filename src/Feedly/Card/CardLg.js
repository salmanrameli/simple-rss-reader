...
class CardLg extends Component {
    ...
    render() {
        return(
            <div class={`grid-item span-32 vw${this.state.item.indexKey % 30} cursor-pointer ${this.state.activeLink === this.state.item.id ? "text-white bg-primary" : this.state.item.unread === true ? 'text-dark' : 'text-secondary'}`} key={this.state.item.id}>
                <div onClick={(e) => this.handleMarkAsRead(e, this.state.item.canonicalUrl, this.state.item.id, true, this.state.item.unread, true, this.state.item.webfeeds ? this.state.item.webfeeds.wordmark !== undefined ? this.state.item.webfeeds.wordmark : this.state.item.webfeeds.logo !== undefined ? this.state.item.webfeeds.logo : null : null)} className="cursor-pointer">
                    ...
                    <div className="actions w-100">
                        ...
                        <button type="button" onClick={() => this.openInBrowser(this.state.item.canonicalUrl)} title="Open article in browser" className="no-focus browser-icon m-auto">
                            <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 24 24">
                        ...
                        <button type="button" onClick={() => this.copyUrl(this.state.item.canonicalUrl)} title="Copy URL address" className="no-focus url-icon m-auto">
                            <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 24 24">
